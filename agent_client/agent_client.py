...
class rsvp_client_api:
    msg_id = 0
    def __init__(self, ip_addr=API_DST_ADDRESS, port=API_MANAGEMENT_PORT):
...
def main():
    ...
    print '\nParsing arguments...'
    conf = rsvp_conf()
    conf = find_arguments(sys.argv[2:], conf)
    if command == 'create':
        print 'Sent CREATE REQ message to 127.0.0.1:'+ str(API_MANAGEMENT_PORT) + ':', conf
    elif command == 'delete':
        print 'Sent DELETE REQ message to 127.0.0.1:'+ str(API_MANAGEMENT_PORT) + ':', conf
    elif command == 'status':
        print 'Sent STATUS REQ message to 127.0.0.1:'+ str(API_MANAGEMENT_PORT)
    ...
    if command == 'create':
        ...
        client.destroy_path(conf)
    elif command == 'status':
        client.status_query(conf)
    else:
        print 'Unproper usage of the agent client!'
        present_usage()
